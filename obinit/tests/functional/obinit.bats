...
@test "obinit should clear upper if configured" {
 cp "$TESTS_DIR/configs/overboot-volatile.yaml" "$TEST_ROOTMNT_DIR/etc/overboot.yaml"
...
#   testFileName="$(date +%s).test"
#   touch "$TEST_DURABLES_DIR_1/$testFileName"
  
#   [ ! -f "$TEST_DURABLES_DIR_1/orig.txt" ]
#   [ -f "$TEST_DURABLES_STORAGE_DIR/test/durables-1/$testFileName" ] # TODO: fix paths
# }
...
#   testFileName="$(date +%s).test"
#   touch "$TEST_DURABLES_DIR_2/$testFileName"
  
#   [ -f "$TEST_DURABLES_DIR_2/orig.txt" ]
#   [ -f "$TEST_DURABLES_STORAGE_DIR/test/durables-2/$testFileName" ] # TODO: fix paths
# }

# @test "obinit should bind durable directories and skip copying origin if the storage is not empty" {
#   $OBINIT_BIN -r "$TEST_RAMFS_DIR"

#   # TODO: fix paths
#   testFileName="$(date +%s).test"
#   mkdir -p "$TEST_DURABLES_STORAGE_DIR/test/durables-2"
#   touch "$TEST_DURABLES_STORAGE_DIR/test/durables-2/$testFileName"
  
#   [ ! -f "$TEST_DURABLES_DIR_2/orig.txt" ]
#   [ -f "$TEST_DURABLES_STORAGE_DIR/test/durables-2/$testFileName" ] # TODO: fix paths
# }

# @test "obinit should bind durable files" {
#  skip
# }
